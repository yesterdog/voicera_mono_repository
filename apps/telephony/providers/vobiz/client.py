"""Vobiz HTTP client — auth headers and base URL."""
...
class VobizClient:
    """Thin client for Vobiz Application, outbound Call, and Recording APIs.

    Credentials and base URL are injected — no org/Integrations lookup.
    """

    PROVIDER_LABEL = "Vobiz"
    ...
    def auth_headers(self, *, include_content_type: bool = True) -> Dict[str, str]:
        headers = {
            "X-Auth-ID": self.credentials.auth_id,
            "X-Auth-Token": self.credentials.auth_token,
        }
    ...
    def account_url_lower(self, *parts: str) -> str:
        """Some Vobiz number endpoints use lowercase ``account``."""
        segments: list[str] = []
        trailing_slash = False
        for p in parts:
            if not p:
...
def client_or_fail(
    ...
    base_url: str,
) -> tuple[Optional[VobizClient], Optional[Dict[str, Any]]]:
    """Build a client or return a fail dict (for callers with optional creds)."""
    try:
        return VobizClient(auth_id or "", auth_token or "", base_url), None
    except ValueError:
        return None, missing_credentials_result(VobizClient.PROVIDER_LABEL)
